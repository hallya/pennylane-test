...
function findDependents(project, filePath) {
  ...
  const modifiedDir = absoluteFilePath.substring(0, absoluteFilePath.lastIndexOf('/'));
...
function main() {
  ...
  const relativeFiles = allRelatedFiles.map(file => file.startsWith(cwd) ? file.substring(cwd.length + 1) : file);
